...
Quilt lets you manage execution engines, monitor pipeline performance, and audit data lineage directly from the user interface.
...
* **Engine Indicator**: The status bar shows the active execution engine, **DuckDB**.
...
* **Plan Tab**: Shows the exact, compile-time SQL script Quilt built from your visual layout. You can copy this script to execute it directly inside other database tools.
...
Quilt pre-fetches database components on startup so that your visual connectors can execute pipelines without an active internet connection.
