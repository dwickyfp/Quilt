        ...
        for stage in &compiled.stages {
            ...
            for attempt in 0..stage.retry_attempts {
                ...
                result = match stage.runtime.as_ref() {
                    ...
                    Some(RuntimeSpec::MlForecastEts(spec)) => self.run_ml_forecast_ets(&db_path, spec),
                    Some(RuntimeSpec::MlForecastAutoArima(spec)) => self.run_ml_forecast_auto_arima(&db_path, spec),
                    Some(RuntimeSpec::MlTimeseriesDecompose(spec)) => self.run_ml_timeseries_decompose(&db_path, spec),
                    Some(RuntimeSpec::XfStatOutlier(spec)) => self.run_xf_stat_outlier(&db_path, spec),
