        ...
        groups: [
            ...
            },
            {
                ...
                components: [
                    ...
                    xf('stat.outlier', 'Outlier Detection', 'available', 'Flag outliers in numeric columns using IQR or Z-score method. Props: columns (numeric columns to check; empty = all numeric), method (iqr|zscore, default iqr), threshold (Z-score threshold, default 3.0). Outputs: original data + {col}_outlier (0/1) flag columns.'),
                ],
            },
            {
                id: 'xf.pivot',
        ...
        groups: [
            ...
            },
            {
                ...
                components: [
                    ...
                    ml('ml.learner.ridge', 'Ridge Regression Learner', 'available', 'Train an L2-regularized linear regression (numeric target). Props: targetColumn (numeric), featureColumns, alpha (regularization strength, default 1.0). Outputs a model on the model port.'),
                    ml('ml.learner.lasso', 'Lasso Regression Learner', 'available', 'Train an L1-regularized linear regression that drives weak coefficients to zero (feature selection). Props: targetColumn (numeric), featureColumns, alpha (default 1.0). Outputs a model on the model port.'),
                    ml('ml.learner.elasticnet', 'ElasticNet Learner', 'available', 'Train an ElasticNet regression blending L1 and L2 penalties. Props: targetColumn (numeric), featureColumns, alpha (default 1.0), l1Ratio (0=ridge .. 1=lasso, default 0.5). Outputs a model on the model port.'),
                    ml('ml.learner.dbscan', 'DBSCAN Learner', 'available', 'Fit a DBSCAN density clustering model (unsupervised). Finds arbitrarily shaped clusters and marks outliers as noise; no k needed. Props: featureColumns, eps (neighborhood radius, default 0.5), minSamples (default 5). Outputs a model on the model port; predict appends a cluster id (noise = its own group).'),
                    ml('ml.learner.nb.gaussian', 'Gaussian Naive Bayes Learner', 'available', 'Train a Gaussian Naive Bayes classifier (assumes per-class normal features). Props: targetColumn (categorical), featureColumns. Fast probabilistic baseline. Outputs a model on the model port.'),
                    ml('ml.learner.xgb', 'Gradient Boosting Classifier', 'available', 'Train a gradient-boosted decision tree classifier (binary, gbdt). Props: targetColumn (exactly 2 classes), featureColumns, nTrees (boosting iterations, default 100), maxDepth, learningRate (shrinkage, default 0.1). Outputs a model on the model port; predict appends the predicted class.'),
                    ml('ml.learner.xgb.reg', 'Gradient Boosting Regressor', 'available', 'Train a gradient-boosted decision tree regressor (numeric target, gbdt). Props: targetColumn (numeric), featureColumns, nTrees (boosting iterations, default 100), maxDepth, learningRate (shrinkage, default 0.1). Outputs a model on the model port; predict appends a numeric prediction.'),
                    ml('ml.learner.mlp', 'MLP Neural Network', 'available', 'Multi-layer perceptron classifier via SGD backprop. Props: targetColumn, featureColumns, hiddenLayerSize (default 100 via k), learningRate (default 0.01), maxIter (default 200). Binary + multi-class.'),
                    ml('ml.learner.svc', 'Support Vector Classifier', 'available', 'Train a support vector machine classifier (binary). Props: targetColumn, featureColumns, kernel (linear|rbf, default rbf), c (regularization, default 1.0), gamma (kernel coeff, default 0 = auto). Outputs a model on the model port. ⚠️ Recommended: pipe features through xf.num.zscore first — SVM is sensitive to feature scales; unscaled data (e.g. area 0-500 vs bedrooms 0-5) causes degenerate predictions.'),
                    ml('ml.learner.svr', 'Support Vector Regressor', 'available', 'Train a support vector machine regressor (numeric target). Props: targetColumn (numeric), featureColumns, kernel (linear|rbf, default rbf), c (regularization, default 1.0), epsilon (margin width, default 0.1), gamma (kernel coeff, default 0 = auto). Outputs a model on the model port. ⚠️ Recommended: pipe features through xf.num.zscore first — SVM is sensitive to feature scales; unscaled data causes poor R².'),
                ],
            },
            {
                id: 'ml.apply',
                ...
                components: [
                    ...
                    ml('ml.feature.importance', 'Feature Importance', 'available', 'Extract feature importance from a trained model (model port). For linear models: absolute coefficients. For tree models: split usage counts. For GBDT: feature_index counts. Outputs a table sorted by importance (feature, importance, percentage). Supports: linreg, ridge, lasso, elasticnet, logreg, tree, forest, xgb, xgb.reg.'),
                    ml('ml.anomaly.isolation_forest', 'Isolation Forest', 'available', 'Anomaly detection via Isolation Forest ensemble. Props: featureColumns, nTrees (default 100), maxDepth (0=auto), contamination (default 0.1). Outputs: original data + anomaly_score + is_anomaly (0/1).'),
                    ml('ml.forecast.arima', 'ARIMA Forecaster', 'available', 'Time series forecasting using ARIMA(p,d,q) model. Props: targetColumn (numeric time series), p (AR order, default 1), d (differencing order, default 1), q (MA order, default 0), steps (forecast horizon, default 10). Outputs: original data + fitted values + forecast + 95% confidence intervals. Supports AR, MA, and combined ARMA models with differencing for trend removal.'),
                    ml('ml.forecast.ets', 'ETS Exponential Smoothing', 'available', 'Exponential smoothing forecasting: SES (simple), Holt (trend), or Holt-Winters (trend+seasonal). Props: targetColumn, alpha (level smoothing, default 0.3), beta (trend smoothing, default 0.1), gamma (seasonal smoothing, default 0.1), seasonalPeriod (0=no seasonal), steps (forecast horizon, default 10), trend (boolean, default true). Outputs: original data + fitted + forecast + 95% CI.'),
                    ml('ml.forecast.auto.arima', 'Auto-ARIMA', 'available', 'Automatic ARIMA model selection via grid search over (p,d,q) with AIC minimization. Props: targetColumn, maxP (default 5), maxD (default 2), maxQ (default 5), steps (forecast horizon, default 10). Outputs: best model forecast + fitted values + 95% CI.'),
                    ml('ml.timeseries.decompose', 'Time Series Decompose', 'available', 'Classical time series decomposition (additive or multiplicative). Props: targetColumn, period (seasonal period, required >=2), model (additive|multiplicative, default additive). Outputs: original data + trend + seasonal + residual columns.'),
                    ...
                    ml('ml.featureselect', 'Feature Selector', 'available', 'Greedy forward feature selection driven by k-fold CV. Starting from no features, repeatedly adds the candidate that most improves the mean CV score (accuracy up / RMSE down), stopping when none improves it or maxFeatures is reached. Emits the selection path: step, added feature, running feature set, cv score. Props: algorithm, targetColumn, featureColumns (candidate pool; blank = all other columns), folds, seed, maxFeatures (0 = no cap), task, plus learner hyperparams. Wire the data into main.'),
                     ml('ml.gridsearch', 'Grid Search', 'available', 'Exhaustive hyperparameter tuning via grid search over k-fold cross-validation. Specify a JSON paramGrid (e.g. `{"maxDepth":[3,5,7],"nTrees":[10,50,100]}`) and the node evaluates every combination, reporting a ranked results table with best params. Props: algorithm, targetColumn, featureColumns, folds, seed, task, paramGrid (JSON string). Wire the data into main.'),
                    ml('ml.pca', 'PCA', 'available', 'Principal component analysis — a single fit+transform node that reduces the chosen numeric feature columns to N principal components and appends them as new columns (pc1..pcN). Props: featureColumns (blank = all numeric columns), nComponents (default 2), useCorrelationMatrix (standardize variables, default false), outputPrefix (default `pc`), dropFeatures (drop the source columns, default false). Wire the data into main.'),
                    ml('ml.onehot', 'One-Hot Encoder', 'available', 'One-hot encode categorical columns — a single transform node that appends one 0/1 indicator column per distinct value (<col>_<value>). Props: columns (categorical columns to encode, required), maxCategories (cap distinct values per column; rare ones fold into <col>_other, 0 = no cap), dropOriginal (drop the source columns, default false). Wire the data into main.'),
                    // Text Mining
                    ml('tm.tokenize', 'Tokenizer', 'available', 'Split text column into individual tokens (one row per token). Props: textColumn (required), lowercase (default true), minLength (default 2), stopwords (default true — removes 180+ English stopwords), delimiter (default whitespace). Output: all original columns + token column.'),
                    ...
                    // SHAP / XAI
                    ml('ml.explain.shap', 'SHAP Explainer', 'available', 'SHAP (SHapley Additive exPlanations) for model interpretability. Props: modelNode (required — node ID of trained model), featureColumns (required), backgroundSamples (default 100). For linear models: exact LinearExplainer. For tree/ensemble: permutation-based SHAP. Output: original data + shap_{feature} columns.'),
                ],
            },
            {
                id: 'ml.export',
